...
namespace
{
	...
	int __fastcall ForgeUpdateHook(void *thisptr, void* unused, int a2);
	int  __cdecl MoveObjectHook(float f, int a, int b, int c, int d);
}
...
namespace Patches
{
	namespace Forge
	{
		void ApplyAll()
		{
			...
			Hook(0x19D43C, ForgeUpdateHook, HookFlags::IsCall).Apply();
			Hook(0x19CAFC, MoveObjectHook, HookFlags::IsCall).Apply();
...
namespace
{
	int  __cdecl MoveObjectHook(float f, int a, int b, int c, int d)
	{
		typedef int(__cdecl *OriginalFuncPtr)(float f, int a, int b, int c, int d);
		auto originalFunc = reinterpret_cast<OriginalFuncPtr>(0x0059E340);

		::Forge::Tool::OnMoveObject(a);

		return originalFunc(f, a, b, c, d);
	}

	int __fastcall ForgeUpdateHook(void *thisptr, void* unused, int a2)
	{
		typedef int(__thiscall *OriginalFuncPtr)(void *thisptr, int a2);
		auto originalFunc = reinterpret_cast<OriginalFuncPtr>(0x0074A200);

		::Forge::Tool::Update();

		return originalFunc(thisptr, a2);
	}
